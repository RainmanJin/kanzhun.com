(function (factory) {
  ...
  validator.reg = {
    ...
    isUrl:function(str){
      return (/^(?:(?:ht|f)tp(?:s?)\:\/\/|~\/|\/)?(?:\w+:\w+@)?((?:(?:[-\w\d{1-3}]+\.)+(?:com|org|net|gov|mil|biz|info|mobi|name|aero|jobs|edu|co\.uk|ac\.uk|it|fr|tv|museum|asia|local|travel|[a-z]{2}))|((\b25[0-5]\b|\b[2][0-4][0-9]\b|\b[0-1]?[0-9]?[0-9]\b)(\.(\b25[0-5]\b|\b[2][0-4][0-9]\b|\b[0-1]?[0-9]?[0-9]\b)){3}))(?::[\d]{1,5})?(?:(?:(?:\/(?:[-\w~!$+|.,=]|%[a-f\d]{2})+)+|\/)+|\?|#)?(?:(?:\?(?:[-\w~!$+|.,*:]|%[a-f\d{2}])+=?(?:[-\w~!$+|.,*:=]|%[a-f\d]{2})*)(?:&(?:[-\w~!$+|.,*:]|%[a-f\d{2}])+=?(?:[-\w~!$+|.,*:=]|%[a-f\d]{2})*)*)*(?:#(?:[-\w~!$ |\/.,*:;=]|%[a-f\d]{2})*)?$/i).test(str);
    }
  };
  ...
  validator.prototype = {
    ...
    validateText: function (prompt, _this, e, submitFn) {
      ...
      if ( (val === '' || (placeholder && val == placeholder))) {  
        ...
        //此处的添加了$(this).data('vname')，针对name是变量的情况
        var more = _this.more[$(this).data('vname') || this.name] || {},
          type = more.type,
          msg = more.msg;
        ...
        /*more{
          xxx: function(){} //不用处理prompt
          xxx: {
            fn: function(){} //需要处理prompt
          }
          xxx: {
            type: ExpReg,
            msg: string,
            fn: function(){} //不需要处理prompt
          }
        }
        */
  ...
  validatorAll.prototype.submit = function (fn) {
    ...
    //同时绑定自定义方法likesubmit 为非表单元素使用
    this.form.on('submit likesubmit', function (event) {
