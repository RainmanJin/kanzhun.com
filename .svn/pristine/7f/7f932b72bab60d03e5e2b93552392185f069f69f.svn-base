...
//  };
  ...
  comment.prototype = {
    ...
    bindEvents: function(){
      ...
      }).*/on('keyup', 'div.js-autotextarea', function(){
          var start_ptn = /<\/?[^>]*>/g,     //过滤标签开头
              end_ptn = /[ | ]*\n/g,        //过滤标签结束
              space_ptn = /&nbsp;/ig;          //过滤空格
          var texVal = $(this).html().replace(start_ptn, "").replace(end_ptn).replace(space_ptn, " ");
          var remain = texVal.length - 140;
          msg = $(this).parent().find('>p>span.cmt_w');
      ...
      //publish
      }).on('click', 'span.splaceholder', function(){
          var self = $(this), placeholderText = self.parent().find('>p');
          self.prev('div').focus();
          if( placeholderText.is(':hidden') ){
            placeholderText.show();
          }
          self.addClass('myhide');

        }).on('click', 'span.pub_cmt', function(){
        var self = $(this),
          start_ptn = /<\/?[^>]*>/g,     //过滤标签开头
          end_ptn =  /<\/{1}([a-zA-Z1-6]+)>/g,        //过滤标签结束
          space_ptn = /&nbsp;/ig,          //过滤标签空格
          wrap = self.parent().parent(),
          field = wrap.find('span.splaceholder'),
          iptDiv = field.prev('div.js-autotextarea'),
          val = iptDiv.html().replace(/<\/?[^>]*>/, "\r\n").replace(end_ptn, "\r\n").replace(start_ptn, "").replace(space_ptn, ""),

          button = $('input', self),
          rContext = self.parents('div.comments_con');
        if(val === '' || val.length > 140){
          iptDiv.addClass('shine').one('animationend webkitAnimationEnd', function(){
            $(this).removeClass('shine');
          });
          if( wrap.find($('span.angle_inner')).length > 0 ){
      ...
      //add reply
        ...
        if(reply.length){
          ...
          parent.append('<div class="reply"><div class="content_ipt js-autotextarea" aria-label="请输入评论内容" contenteditable="true" tabindex="-1"></div><span class="splaceholder">点击输入内容</span>' +
            '<p class="t-right clear none"><b class="red mr20 none">发布失败 : ( </b><span class="cmt_w mr20">您还可以输入<strong class="ok">140</strong>字</span><a href="javascript:;" class="cancel_reply mr20">取消</a>' +
      ...
      //cancel
        ...
        $(this).parent().parent().find('span').removeClass('myhide');
          $(this).parent().parent().find('div.js-autotextarea').html('');
      //delete
